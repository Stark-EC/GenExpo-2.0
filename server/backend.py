...
class Backend_Api:
    def __init__(self, bp, config: dict) -> None:
        ...
        # Load dataset when initializing
    ...
    def _conversation(self):
        ...
        try:
            jailbreak = request.json['jailbreak']
            model = request.json['model']
            messages = build_messages(jailbreak, self.dataset)  # Pass dataset to build_messages
...
def load_dataset():
    with open('server/sample_dataset.json', 'r') as file:
        dataset = json.load(file)
    return dataset['intents']

def build_messages(jailbreak, dataset):
    """  
    Build the messages for the conversation.  
    ...
    # Add responses from the dataset based on the prompt
    # Sample logic: Check prompt content against dataset intents and add corresponding response
    for intent in dataset:
        if prompt["content"].lower() in intent["patterns"]:
            conversation.append({"role": "bot", "content": intent["responses"][0]})  # Assuming first response
